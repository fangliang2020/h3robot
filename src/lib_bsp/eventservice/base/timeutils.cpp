...
class TimeHelper final {
 public:
  ...
  static void SyncWithNtp(int64_t ntp_server_time_ms) {
    ...
    singleton.app_start_time_ns_ =
      (ntp_server_time_ms - kNTPTimeToUnixTimeEpochOffset) * 1000000 - time_zone_bias_ns;
  ...
  static int64_t TicksNs() {
    ...
    result =
      rtc::dchecked_cast<int64_t>((rtc::dchecked_cast<uint64_t>(qpcnt.QuadPart) * 100000 /
                                   rtc::dchecked_cast<uint64_t>(singleton.os_ticks_per_second_)) *
                                  10000);
    result = singleton.app_start_time_ns_ + result - singleton.time_since_os_start_ns_;
    return result;
  }
 ...
 private:
  TimeHelper() {
    ...
    app_start_time_ns_ = (li.QuadPart - kFileTimeToUnixTimeEpochOffset) * 100 - time_zone_bias_ns;
 ...
 private:
  static constexpr uint64_t kFileTimeToUnixTimeEpochOffset = 116444736000000000ULL;
...
TimestampWrapAroundHandler::TimestampWrapAroundHandler() : last_ts_(0), num_wrap_(-1) {}
...
int64_t TmToSeconds(const tm& tm) {
  ...
  if (day < 0 || day >= mdays[month] + (expiry_in_leap_year && month == 2 - 1)) return -1;
...
int64_t TimeUTCMicros() {
  ...
  return (static_cast<int64_t>(time.tv_sec) * chml::kNumMicrosecsPerSec + time.tv_usec);
