...
int aesEncrypt(const uint8_t *key, uint32_t keyLen, const uint8_t *pt, uint8_t *ct, uint32_t len) {
    ...
    if (NULL == key || NULL == pt || NULL == ct){
        printf("Encrypt param err.\n");
        return -1;
    }

    if (keyLen > 16){
        printf("Encrypt keyLen must be 16.\n");
        return -1;
    }

    if (len % BLOCKSIZE){
        printf("Encrypt inLen is invalid.\n");
        return -1;
    }
...
int aesDecrypt(const uint8_t *key, uint32_t keyLen, const uint8_t *ct, uint8_t *pt, uint32_t len) {
    ...
    if (NULL == key || NULL == ct || NULL == pt){
        printf("Decrypt param err.\n");
        return -1;
    }

    if (keyLen > 16){
        printf("Decrypt keyLen must be 16.\n");
        return -1;
    }

    if (len % BLOCKSIZE){
        printf("Decrypt inLen is invalid.\n");
        return -1;
    }
...
int aesEncryptStr(std::string& src, std::string& dst, const uint8_t *key) {
    std::vector<uint8_t> v_src, v_dst;
    v_src.assign(src.begin(), src.end());
    int src_len = v_src.size();
    int remainder = src_len % BLOCKSIZE;
    if (remainder) {
        remainder = BLOCKSIZE - remainder;
        src_len += remainder;
        for (int i = 0; i < remainder; ++i) {
            v_src.emplace_back(0);
        }
    }
    v_dst.resize(src_len, 0);
    aesEncrypt(key, 16, v_src.data(), v_dst.data(), src_len);
    chml::Base64::EncodeFromArray(v_dst.data(), v_dst.size(), &dst);

    return 0;
}

int aesDecryptStr(std::string& src, std::string& dst, const uint8_t *key) {
    src = chml::Base64::Decode(src, chml::Base64::DO_STRICT);
    std::vector<uint8_t> v_src, v_dst;
    v_src.assign(src.begin(), src.end());
    int src_len = v_src.size();
    v_dst.resize(src_len, 0);
    aesDecrypt(key, 16, v_src.data(), v_dst.data(), src_len);
    dst.assign(v_dst.begin(), v_dst.end());
