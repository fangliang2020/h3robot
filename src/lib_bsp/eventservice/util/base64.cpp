...
uint32 Base64::CalcSrcSize(uint32 dst_data_size) {
  // 3的倍数往小了取
  uint32 src_size = dst_data_size * 3 / 4;
  return (src_size - src_size % 3);
}
...
void Base64::EncodeFromArray(MemBuffer::Ptr data, std::string* result,
  ...
  chml::BlocksPtr blks = data->blocks();
  chml::BlocksPtr::iterator iter = blks.begin();
