...
PhysicalSocket::PhysicalSocket(SOCKET s)
  ...
  // initialized by constructor of a static object. If neccessary chml
  ...
  if (s_ != INVALID_SOCKET) {
    ...
    ML_VERIFY(0 == getsockopt(s_, SOL_SOCKET, SO_TYPE, (SockOptArg)&type, &len));
...
int PhysicalSocket::Bind(const SocketAddress& bind_addr) {
  ...
  UpdateLastError();
  if (err == -1) {
    DLOG_ERROR(MOD_EB, "family:%d socket bind failed, error:%s", bind_addr.family() , strerror(error_));
  }
#ifdef _DEBUG
  if (0 == err) {
    dbg_addr_ = "Bound @ ";
...
int PhysicalSocket::Send(const void *pv, size_t cb) {
  //Log_HexDump("PhysicalSocket send ", (unsigned char*)pv, cb);
...
int PhysicalSocket::Send(MemBuffer::Ptr buffer) {
  ...
  return res;
#elif defined(LITEOS)
  char *send_buff = (char*)malloc(buffer->size());
  if (NULL == send_buff) {
  ...
  struct iovec *vec = (struct iovec*)malloc(block_num*sizeof(struct iovec));
  if (NULL == vec) {
...
int PhysicalSocket::Recv(void* buffer, size_t length) {
  ...
  //Log_HexDump("PhysicalSocket recv ", (unsigned char*)buffer, received);
  return received;
}
...
Socket::Ptr PhysicalSocket::Accept(SocketAddress *out_addr) {
  ...
  UpdateLastError();
  if (s == INVALID_SOCKET) {
    DLOG_ERROR(MOD_EB, "accept error: %s", strerror(errno));
    return Socket::Ptr();
  }
  enabled_events_ |= DE_ACCEPT;
  if (out_addr != NULL) {
...
int PhysicalSocket::TranslateOption(Option opt, int* slevel, int* sopt) {
  ...
  case OPT_MULTICAST_MEMBERSHIP:
    ...
    *sopt = IP_ADD_MEMBERSHIP;
    break;
  case OPT_MULTICAST_LOOP:
    *slevel = IPPROTO_IP;
    *sopt = IP_MULTICAST_LOOP;
    break;
  case OPT_REUSEADDR:
    *slevel = SOL_SOCKET;
    *sopt = SO_REUSEADDR;
...
#else
#ifdef POSIX //LITEOS
static int ClockOfDay(struct timeval* tv, void* /*tzv*/) {
#if defined(POSIX)
  struct timespec ts;
