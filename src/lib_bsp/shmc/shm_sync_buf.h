...
class ShmSyncBuf {
 public:
   ...
   */
  bool InitForWrite(const std::string& shm_key, size_t buf_size_bytes);
 ...
 private:
  ...
  bool IsValidNodeHeadPos(uint64_t pos) const {
    return (IsValidPos(pos) && ((pos & 0x7UL) == 0) &&
  ...
  uint64_t PosOff(uint64_t pos_to, uint64_t pos_from) const {
    uint64_t size = shm_->sync_buf_size;
    return (pos_to >= pos_from ? pos_to - pos_from : pos_to + size - pos_from);
  }
};

template <class Alloc>
bool ShmSyncBuf<Alloc>::InitForWrite(const std::string& shm_key, size_t buf_size_bytes) {
  ...
  size_t shm_size = sizeof(ShmHead) + sizeof(uint32_t) * seq_index_size + buf_size_bytes;
  ...
  if (shm_.is_newly_created()) {
    ...
    shm_->next_seq = ((uint64_t)time(NULL) << 32UL);
    shm_->head_pos = 0;
    shm_->tail_pos = 0;
...
bool ShmSyncBuf<Alloc>::InitForRead(const std::string& shm_key) {
  ...
  size_t shm_size = sizeof(ShmHead) + sizeof(uint32_t) * shm_->seq_index_size + shm_->sync_buf_size;
...
bool ShmSyncBuf<Alloc>::Push(const void* buf, size_t len, const timeval& ts) {
  ...
  if (tail_pos + ntail_off <= sync_buf_size) {
    ...
    memcpy(const_cast<char*>(sync_buf_ptr()), static_cast<const char*>(buf) + part_len,
           len - part_len);
  }
  // write new node tail
...
bool ShmSyncBuf<Alloc>::FindBySeq(uint64_t seq, SyncIter* it) const {
  ...
  if (!node_head) {
    Utils::Log(kWarning,
               "ShmSyncBuf::FindBySeq: GetSyncNodeHead "
               "for seq(0x%lx) fail\n",
               seq);
    return false;
  }
...
bool ShmSyncBuf<Alloc>::FindByTime(const timeval& ts, SyncIter* it) const {
  ...
  while (seq_left < seq_right) {
    ...
    if (!FindBySeq(seq_mid, &iter) || (Read(iter, &meta) != 1) || timercmp(&meta.time, &ts, <)) {
      seq_left = seq_mid + 1;
    } else {
      seq_right = seq_mid;
    }
  }
  if (!FindBySeq(seq_right, &iter) || (Read(iter, &meta) != 1) || timercmp(&meta.time, &ts, <)) {
    return false;
  }
  *it = iter;
  return true;
...
template <class Alloc>
int ShmSyncBuf<Alloc>::Read(const SyncIter& it, SyncMeta* meta, void* buf, size_t* len) const {
  assert(shm_.is_initialized());
  uint64_t tail_pos = shm_->tail_pos;
  if (it.pos == tail_pos) {
  ...
  *len = copy_len;
  if (it.pos + GetNodeTailOffset(data_len) <= sync_buf_size) {
    ...
    memcpy(static_cast<char*>(buf) + first_copy_len, const_cast<char*>(sync_buf_ptr()),
...
template <class Alloc>
int ShmSyncBuf<Alloc>::Read(const SyncIter& it, SyncMeta* meta, std::string* out) const {
  size_t len = 0;
  int r;
