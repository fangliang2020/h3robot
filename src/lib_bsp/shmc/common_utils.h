...
static void SHMC_Log_Print(shmc::LogLevel lv, const char* info) {
#ifdef USE_SYS_LOG
  std::map<int, int> loglevel = {
      {shmc::kError, LOG_ERR},
      {shmc::kWarning, LOG_WARNING},
      {shmc::kInfo, LOG_INFO},
      {shmc::kDebug, LOG_DEBUG},
  };
  syslog(loglevel[lv], info);
#else
  printf("### Level:%d -> %s", lv, info);
#endif
}
...
class Utils {
 public:
  ...
  static void SetLogHandler(LogLevel lv, std::function<void(LogLevel, const char*)> f);
...
const char* Utils<Dummy>::Perror() {
  ...
  buf[0]        = 0;
  const char* s = strerror_r(errno,
                             buf,
                             sizeof(buf));
  return (s == nullptr || (int64_t)s == -1L) ?
         buf :
         s;
}
...
template <class Dummy>
bool Utils<Dummy>::GetPrimeArray(size_t top,
                                 size_t num,
                                 size_t array[]) {
  uint64_t i = 0;
  for (; top > 0 && num > 0; top--) {
...
uint64_t Utils<Dummy>::GenMagic(const char* str) {
  uint64_t magic = 0;
  strncpy(reinterpret_cast<char*>(&magic),
          str,
          sizeof(magic));
  return magic;
}
...
template <class Dummy>
const char* Utils<Dummy>::Hex(const volatile void* buf,
                              size_t               len) {
 ...
 */
inline void SetLogHandler(LogLevel log_level, std::function<void(LogLevel, const char*)> log_func) {
