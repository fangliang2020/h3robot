...
#include "eventservice/util/crc32.h"
...
class ShmLinkTable {
 ...
 private:
  ...
  struct BufHead {
    ...
    volatile uint32_t crc_check;
  } __attribute__((__packed__));
  static_assert(sizeof(BufHead) == 8, "unexpected BufHead layout");
...
link_buf_t ShmLinkTable<Alloc>::New(const void* buf, size_t size) {
  ...
  uint32_t crc = chml::MyCrc32::crc32_my(0L, NULL, 0);
  ...
  for (size_t i = 0; i < need_nodes; i++) {
    ...
    if (i == 0) {  // first node
      ...
      size_t check_len = (dst_left <= src_left ? dst_left : src_left);
      buf_head->buf_len = size;
      buf_head->crc_check = chml::MyCrc32::crc32_my(crc, src_ptr, check_len);
    }
    // copy data
...
bool ShmLinkTable<Alloc>::Read(link_buf_t link_buf, void* buf, size_t* size) const {
  ...
  //printf("dst_ptr: %p, copy_len: %d, data_size: %d\n", dst_ptr, copy_len, data_size);
  memcpy(dst_ptr, const_cast<const char*>(head_node->user_node + sizeof(BufHead)),
         copy_len);
  crc = chml::MyCrc32::crc32_my(crc, (const char*)(head_node->user_node + sizeof(BufHead)), cur_node_data_size);
  if (crc != buf_head->crc_check) {
    Utils::Log(kWarning, "crc check failed, crc(0x%x) != buf_head->crc(0x%x), node may be overwritten\n",
               crc, buf_head->crc_check);
    return false;
  }
  left_size -= copy_len;
  dst_ptr += copy_len;
