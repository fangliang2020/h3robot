 ...
 * @Alloc    shm allocator to use [SVIPC(default), SVIPC_HugeTLB, POSIX, ANON,
 * HEAP]
 *
...
class ShmHashMap {
 public:
   ...
   */
  bool InitForWrite(const std::string& shm_key, size_t key_num, size_t val_node_size, size_t val_node_num);
  ...
  /*  Travel all key-values and erase 
  * @f                 callback to be called to Judge whether to delete
  * @max_travel_nodes  max nodes to travel in this method call, 0 for all
  * @return            true if succeed
  */
  bool TravelEraseIf(std::function<bool(const KeyType&, const std::string&)> f,
                     size_t max_travel_nodes = 0);
  /*  Travel all key-values and erase
  * @pos               [in|out] current position of travel
  * @f                 callback to be called to Judge whether to delete
  * @max_travel_nodes  max nodes to travel in this method call, 0 for all
  * @return            true if succeed
  */
  bool TravelEraseIf(TravelPos* pos,
                     std::function<bool(const KeyType&, const std::string&)> f,
                     size_t max_travel_nodes = 0);
   ...
   */
  size_t size() const { return link_table_.link_bufs(); }
  ...
  /*
  *
  * @return size of free space of HashTable index
  */
  size_t hash_table_free_capacity() const {
    size_t capa = hash_table_.ideal_capacity();
    size_t used = link_table_.link_bufs();
    return capa <= used ? 0 : capa - used;
  }

  bool has_enough_capacity(size_t len) {
    return link_table_.free_nodes() >= link_table_.buf_nodes(len);
  }
  ...
  size_t free_percentage() const {
    return std::min(hash_table_free_percentage(), link_table_free_percentage());
  }

 private:
  bool DoInsert(const KeyType& key, const void* val_buf, size_t val_buf_len,
                bool replace);
  struct HTNode;
  bool DoRead(const KeyType& key, const volatile HTNode* node,
              std::string* val) const;
  bool DoRead(const KeyType& key, const volatile HTNode* node, void* val_buf,
 ...
 private:
  ...
  static_assert(alignof(KeyType) <= 8,
                "align requirement of KeyType can't be met");
...
bool ShmHashMap<KeyType, Alloc>::DoInsert(const KeyType& key,
                                          const void* val_buf,
                                          size_t val_buf_len, bool replace) {
  bool is_found;
  volatile HTNode* key_node = hash_table_.FindOrAlloc(key, &is_found);
  if (!key_node) {
    SHMC_ERR_RET("ShmHashMap::DoInsert: key_node is nullptr\n");
  }
  if (is_found && !replace) {
    SHMC_ERR_RET("ShmHashMap::DoInsert: is_found && !replace\n");
  }
  link_buf_t lb = link_table_.New(val_buf, val_buf_len);
  if (!lb) {
    SHMC_ERR_RET("ShmHashMap::DoInsert: New link_buf fail\n");
  }
  if (is_found) {
    ...
    new (const_cast<KeyType*>(&key_node->key))
        KeyType(key);  // contruct key object
    key_node->link_buf = lb;
  }
  return true;
}
...
bool ShmHashMap<KeyType, Alloc>::DoRead(const KeyType& key,
  ...
  //   INIT -> KEY set -> LB set -> LB reset -> ... -> LB cleared -> KEY cleared
  //   -> INIT
...
bool ShmHashMap<KeyType, Alloc>::Dump(const KeyType& key) const {
  Utils::Log(kInfo,
             "===== ShmHashMap Dump =====\n"
             "pid:%d key:\n%s",
             getpid(), Utils::Hex(&key, sizeof(key)));
  const volatile HTNode* key_node = hash_table_.Find(key);
  if (!key_node) {
...
template <class KeyType, class Alloc>
bool ShmHashMap<KeyType, Alloc>::Travel(
    TravelPos* pos, size_t max_travel_nodes,
    std::function<void(const KeyType&, const std::string&)> f) {
  std::string val;
...
template <class KeyType, class Alloc>
bool ShmHashMap<KeyType, Alloc>::TravelEraseIf(std::function<bool(const KeyType&, const std::string&)> f,
  size_t max_travel_nodes/* = 0*/) {
  TravelPos pos;
  return TravelEraseIf(&pos, f, max_travel_nodes);
}

template <class KeyType, class Alloc>
bool ShmHashMap<KeyType, Alloc>::TravelEraseIf(TravelPos* pos,
  std::function<bool(const KeyType&, const std::string&)> f,
  size_t max_travel_nodes/* = 0*/) {
  std::string val;
  return hash_table_.Travel(pos, max_travel_nodes, [this, &f, &val](volatile HTNode* node){
                              KeyType key = node->key;
                              if (DoRead(key, node, &val)) {
                                if (f(key, val)) {
                                  Erase(key);
                                  // Utils::Log(kInfo, "ShmHashMap::TravelEraseIf: Erase key %llu\n", key);
                                }
                              } else {
                                // if DoRead failed. will be erase
                                Erase(key);
                                // Utils::Log(kInfo, "ShmHashMap::TravelEraseIf: key %llu Read Data Fail, will be Erase\n", key);
                              }
                            });
}
...
bool ShmHashMap<KeyType, Alloc>::HealthCheck(HealthStat* hstat, bool auto_fix) {
  ...
  if (!hash_table_.Travel(
          [this, hstat, auto_fix, &bitmap](volatile HTNode* node) {
